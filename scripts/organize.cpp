...
#include <ros/ros.h>
#include <my_robot/Boundingbox.h>
#include <my_robot/Plot.h>
...
std::string frame_id="camera_color_frame";
pcl::PointCloud<pcl::PointXYZ> pass_filtered_cloud;
static const std::string PCL_TOPIC = "/zed2i/zed_node/point_cloud/cloud_registered";///camera/depth/image_rect_raw /camera/depth/points /zed2i/zed_node/point_cloud/cloud_registered /camera/depth/color/points
...
void callback(const sensor_msgs::PointCloud2ConstPtr& cloud_msg) { 
   ...
   std::cout << "here1\n"<<std::endl;
   //if (start > 0)//if human is present 
   //{
...
// ----------------------------------------
   ...
   vg.setLeafSize(0.09,0.0,0.09); //(0.07,0.0,0.07)
...
void extractObject(pcl::PointCloud<pcl::PointXYZ>::Ptr crop_cloud_ptr)
{
    ...
    SAC_filter.setDistanceThreshold (0.001);//0.001
...
//void human_callback(darknet_ros_msgs::ObjectCount data){
  // start = data.count;
...
int main(int argc, char **argv)
{
   ...
   //subscribe
   //ros::Subscriber yolo_sub = nh.subscribe("found_object", 10, human_callback);
