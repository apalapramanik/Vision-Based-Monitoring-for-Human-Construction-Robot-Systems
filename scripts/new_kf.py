...
class Prediction(object):
    ...
    def __init__(self):
        ...
        self.error_ub_h1 = rospy.Publisher("error_ub_h1", distance, queue_size=1)
        self.error_lb_h1 = rospy.Publisher("error_lb_h1", distance, queue_size=1)
        self.kalfil_ub1 = rospy.Publisher("ub1",position,queue_size=1 )
        self.kalfil_lb1 = rospy.Publisher("lb1",position,queue_size=1 )
        ...
        self.error_ub_h2 = rospy.Publisher("error_ub_h2", distance, queue_size=1)
        self.error_lb_h2 = rospy.Publisher("error_lb_h2", distance, queue_size=1)
        self.kalfil_ub2 = rospy.Publisher("ub2",position,queue_size=1 )
        self.kalfil_lb2 = rospy.Publisher("lb2",position,queue_size=1 )
        ...
        self.error_ub_h3 = rospy.Publisher("error_ub_h3", distance, queue_size=1)
        self.error_lb_h3 = rospy.Publisher("error_lb_h3", distance, queue_size=1)
        self.kalfil_ub3 = rospy.Publisher("ub3",position,queue_size=1 )
        self.kalfil_lb3 = rospy.Publisher("lb3",position,queue_size=1 )
    ...
    def cloud_callback(self,data):
        ...
        if self.indicator == 1:
            ...
            error_ub_h1_msg = distance()
            error_lb_h1_msg = distance()
            kalfil_ub1_msg = position()
            kalfil_lb1_msg = position()
            
            Human_Marker_cube2 = Marker()
            prediction2 = Marker()
            position_msg2 = position()
            prediction_msg2 = position()
            error_ub_h2_msg = distance()
            error_lb_h2_msg = distance()
            kalfil_ub2_msg = position()
            kalfil_lb2_msg = position()
            
            Human_Marker_cube3 = Marker()
            prediction3 = Marker()
            position_msg3 = position()
            prediction_msg3 = position()
            error_ub_h3_msg = distance()
            error_lb_h3_msg = distance()
            kalfil_ub3_msg = position()
            kalfil_lb3_msg = position()
            ...
            '''   
                Plot Gaussian:  Uncomment lines below to see gaussian plots
            '''
    ...
    #------------------------------------------- DB SCAN ------------------------------------------------------------------
            ...
            #start db scan:
            DBSCAN_cluster = DBSCAN(eps=0.5, min_samples=28).fit(xz_np_array) #0.5, 30
            ...
            #ignore empty slice warning when no cluster
            with warnings.catch_warnings():
                ...
                if np.nanmean(x1) != float(nan):       
                    ...
                    pos1 = [meanx1,meanz1] 
                    #-----------------------
                    pos1_old= [meanx1,meanz1,0.0]
                    pos1_trans = quaternion.rotate_vectors(q_rot, pos1_old) + translation
                    print("position1_new:", pos1_trans)
                    #-----------------------
                    # print("position1:", pos1)
                    
                    # publish current human position
                    position_msg1.header = data.header
                    position_msg1.x = pos1_trans[0] #meanx1
                    position_msg1.z = pos1_trans[1]#meanz1
                    self.pose_human1.publish(position_msg1)
                
                if np.nanmean(x2) != float(nan):   
                    ...
                    pos2 = [meanx2,meanz2] 
                    #-----------------------
                    pos2_old= [meanx2,meanz2,0.0]
                    pos2_trans = quaternion.rotate_vectors(q_rot, pos2_old) + translation
                    print("position2_new:", pos2_trans)
                    #-----------------------                    
                    # print("position2:", pos2)
                    
                    #publish current human position
                    position_msg2.header = data.header
                    position_msg2.x = pos2_trans[0] #meanx2
                    position_msg2.z = pos2_trans[1] #meanz2
                    self.pose_human2.publish(position_msg2)
                
                    
                if np.nanmean(x3) != float(nan):   
                    ...
                    pos3 = [meanx3,meanz3] 
                    #-----------------------
                    pos3_old= [meanx3,meanz3,0.0]
                    pos3_trans = quaternion.rotate_vectors(q_rot, pos3_old) + translation
                    print("position3_new:", pos3_trans)
                    #----------------------- 
                    # print("position3:", pos3)
                    
                    #publish current human position
                    position_msg3.header = data.header
                    position_msg3.x = pos3_trans[0] #meanx3
                    position_msg3.z = pos3_trans[1] #meanz3
                    self.pose_human3.publish(position_msg3)
                    
                # #store 10 position coordintes in array for kf:
                if len(human_pos1)<10 :
                    human_pos1.append(pos1_trans[0:2])
                else :
                    human_pos1.pop(0)
                    human_pos1.append(pos1_trans[0:2])
                # print(human_pos1)
                    
                if len(human_pos2)<10 :
                    human_pos2.append(pos2_trans[0:2])
                else :
                    human_pos2.pop(0)
                    human_pos2.append(pos2_trans[0:2])
                    
                if len(human_pos3)<10 :
                    human_pos3.append(pos3_trans[0:2])
                else :
                    human_pos3.pop(0)
                    human_pos3.append(pos3_trans[0:2])
                ...
                Human_Marker_cube1.pose.position.x = pos1_trans[0] #meanx1 
                Human_Marker_cube1.pose.position.y = 0.0
                Human_Marker_cube1.pose.position.z = pos1_trans[1]#meanz1 
                ...
                Human_Marker_cube2.pose.position.x = pos2_trans[0] #meanx2 #transformed_pose.position.x
                Human_Marker_cube2.pose.position.y = 0.0
                Human_Marker_cube2.pose.position.z = pos2_trans[1] #meanz2 #transformed_pose.position.z
                ...
                Human_Marker_cube3.pose.position.x = pos3_trans[0] #meanx3 #transformed_pose.position.x
                Human_Marker_cube3.pose.position.y = 0.0
                Human_Marker_cube3.pose.position.z = pos3_trans[1] #meanz3 #transformed_pose.position.z
    ...
    #             #human 1   
                if np.nanmean(x1) != float(nan):
                    ...
                    if math.isnan(pt[0]):
                        ...
                        if len(prev_pred_x1) <3:
                            prev_pred_x1.append(predicted1[0])
                        else:
                            prev_pred_x1.pop(0)
                            prev_pred_x1.append(predicted1[0])
                            
                        if len(prev_pred_z1) <3:
                            prev_pred_z1.append(predicted1[1])
                        else:
                            prev_pred_z1.pop(0)
                            prev_pred_z1.append(predicted1[1])
                            
                            
                        error_x1 = abs(pos1_trans[0] - prev_pred_x1[0])
                        error_z1 = abs(pos1_trans[1] - prev_pred_z1[1])
                        
                        kalfil_lb1_msg.header = data.header    
                        kalfil_lb1_msg.x = pos1_trans[0] - error_x1
                        kalfil_lb1_msg.z = pos1_trans[1] - error_z1
                        error_lb_h1_msg.header = data.header
                        error_lb_h1_msg.distance = math.sqrt((pos1_trans[0] - error_x1)**2 + (pos1_trans[1] - error_z1)**2)-1.25
                        self.error_lb_h1.publish(error_lb_h1_msg) 
                        self.kalfil_lb1.publish(kalfil_lb1_msg)
                        
                        kalfil_ub1_msg.header = data.header
                        kalfil_ub1_msg.x = pos1_trans[0] + error_x1
                        kalfil_ub1_msg.z = pos1_trans[1] + error_z1
                        self.kalfil_ub1.publish(kalfil_ub1_msg)
                        error_ub_h1_msg.header = data.header
                        error_ub_h1_msg.distance = math.sqrt((pos1_trans[0] + error_x1)**2 + (pos1_trans[1] + error_z1)**2)-1.25
                        self.error_ub_h1.publish(error_ub_h1_msg)
    ...
    #********************************************************************************************************************
                    ...
                    if math.isnan(pt[0]):
                        ...
                        if len(prev_pred_x2) <3:
                            prev_pred_x2.append(predicted2[0])
                        else:
                            prev_pred_x2.pop(0)
                            prev_pred_x2.append(predicted2[0])
                            
                        if len(prev_pred_z2) <3:
                            prev_pred_z2.append(predicted2[1])
                        else:
                            prev_pred_z2.pop(0)
                            prev_pred_z2.append(predicted2[1])
                            
                            
                        error_x2 = abs(pos2_trans[0] - prev_pred_x2[0])
                        error_z2 = abs(pos2_trans[1] - prev_pred_z2[1])
                        
                        
                        error_lb_h2_msg.header = data.header
                        kalfil_lb2_msg.header = data.header    
                        kalfil_lb2_msg.x = pos2_trans[0] - error_x2
                        kalfil_lb2_msg.z = pos2_trans[1] - error_z2
                        error_lb_h2_msg.distance = math.sqrt((pos2_trans[0] - error_x2)**2 + (pos2_trans[1] - error_z2)**2)-1.25
                        self.error_lb_h2.publish(error_lb_h2_msg)                        
                        self.kalfil_lb2.publish(kalfil_lb2_msg)
                        
                        kalfil_ub2_msg.header = data.header
                        kalfil_ub2_msg.x = pos2_trans[0] + error_x2
                        kalfil_ub2_msg.z = pos2_trans[1]  + error_z2
                        error_ub_h2_msg.header = data.header
                        error_ub_h2_msg.distance = math.sqrt((pos2_trans[0] + error_x2)**2 + (pos2_trans[1] + error_z2)**2) - 1.25
                        self.error_ub_h2.publish(error_ub_h2_msg) 
                        self.kalfil_ub2.publish(kalfil_ub2_msg)
                ...
                if np.nanmean(x3) != float(nan):
                    ...
                    if math.isnan(pt[0]):
                        ...
                        error_x3 = abs(pos3_trans[0] - prev_pred_x3[0])
                        error_z3 = abs(pos3_trans[1] - prev_pred_z3[1])
                        
                        kalfil_lb3_msg.header = data.header    
                        kalfil_lb3_msg.x = pos3_trans[0] - error_x3
                        kalfil_lb3_msg.z = pos3_trans[1] - error_z3
                        self.kalfil_lb3.publish(kalfil_lb3_msg)
                        error_lb_h3_msg.header = data.header
                        error_lb_h3_msg.distance = (math.sqrt((pos3_trans[0] - error_x1)**2 + (pos3_trans[1] - error_z1)**2))-1.25
                        self.error_lb_h3.publish(error_lb_h3_msg) 
                        
                        kalfil_ub3_msg.header = data.header
                        kalfil_ub3_msg.x = pos3_trans[0] + error_x3
                        kalfil_ub3_msg.z = pos3_trans[1] + error_z3
                        self.kalfil_ub3.publish(kalfil_ub3_msg)
                        error_ub_h3_msg.header = data.header
                        error_ub_h3_msg.distance = (math.sqrt((pos3_trans[0] + error_x1)**2 + (pos3_trans[1] + error_z1)**2))- 1.25
                        self.error_ub_h3.publish(error_ub_h3_msg) 
...
if __name__ == '__main__':
    main()
                    ...
                    #   if np.size(transform_array1)<10:
                    #     if np.size(transform_array1)==0:
                    #         transform_array1 = np.append(transform_array1, pos1)
                    ...
                    #         transform_array1 = np.append(transform_array1, pos1)
                    # else:
                    #     transform_array1 = np.delete(transform_array1, 0)
                    #     for i in range(len(transform_array1)):
                    #             transform_array1[i] = quaternion.rotate_vectors(q_rot,transform_array1[i]) + translation
                    #     transform_array1 = np.append(transform_array1, pos1)
                        
                    # print("trans 1:", transform_array1)
